				...
				$.each(sub_processes, function (index, row) {
					...
					if (exist_check <= -1) {
						...
						if (row_array[2]) {
							...
							machine_mdl = '<a href="javascript:;" onclick="addProcessMachines(' + main_process + ',' + row_array[2] + ','+data.run_sec_pro_id_array[index]+')" class="btn btn-default btn-xs">Add Item</a>';
							labor_mdl = '<a href="javascript:;" onclick="addProcessLabours(' + main_process + ',' + row_array[2] + ','+data.run_sec_pro_id_array[index]+')" class="btn btn-default btn-xs">Add Item</a>';
							li_id = row_array[2];
						} else {
							...
							machine_mdl = '<a href="javascript:;" onclick="addProcessMachines(' + main_process + ',' + row_array[0] + ','+data.run_sec_pro_id_array[index]+')" class="btn btn-default btn-xs">Add Item</a>';
							labor_mdl = '<a href="javascript:;" onclick="addProcessLabours(' + main_process + ',' + row_array[0] + ','+data.run_sec_pro_id_array[index]+')" class="btn btn-default btn-xs">Add Item</a>';
							li_id = row_array[0];
						}
...
function getSectionTemplate(section_id, section_name, sub_section_id, sub_section_name, section_parents, start_date, end_date,operation_id,run_sec_id) {
	...
	var template = '<div id="section' + section_id + '" class="column section-column section-' + section_id + '">' +
		...
		'<button type="button" style="font-size: 12px;" class="btn btn-default btn-xs remove-icon" onclick="deleteMainSection(this,' + section_id + ',' + sub_section_id + ',' + operation_id + ')"><i class="fa fa-fw fa-close"></i></button>' +
		'</div>' +
		section_name +
		sub_details +
...
function deleteRunSectionStock(run_sec_id,stock_grn_id){
...
function deleteMainSection(ele =null,sec_id,run_sec,operation_id = null){
