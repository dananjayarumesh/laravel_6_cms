$(document).ready(function(){
	...
	$("#addProcessMachine").click(function(event){
		...
		if(machine_id){
			...
			notify.error('Please select machine');
		}
	...
	$('#addProcessLabor').click(function() {
		...
		if(labor_id){
			...
			notify.error('Please select labor');
		}
		...
		$.each( sub_processes, function( index, row ){
			...
			if(exist_check <= -1){
				...
				if(row_array[2]){
					...
					machine_mdl = '<a href="javascript:;" onclick="addProcessMachines('+main_process+','+row_array[2]+')" class="btn btn-default btn-xs">Add Item</a>';
					labor_mdl = '<a href="javascript:;" onclick="addProcessLabours('+main_process+','+row_array[2]+')" class="btn btn-default btn-xs">Add Item</a>';
					li_id = row_array[2]; 
				}else{
					hidden_input = '<input class="process-id-input" name="section_processes['+main_process+'][]" type="hidden" value="'+row_array[0]+'">';
					machine_mdl = '<a href="javascript:;" onclick="addProcessMachines('+main_process+','+row_array[0]+')" class="btn btn-default btn-xs">Add Item</a>';
					labor_mdl = '<a href="javascript:;" onclick="addProcessLabours('+main_process+','+row_array[0]+')" class="btn btn-default btn-xs">Add Item</a>';
					li_id = row_array[0]; 
				}
				...
				output += '<li class="ui-state-default operation-section-process-tile process-li-'+row_array[0]+'">'+
				'<input class="process-id-input" name="section_processes['+main_process+'][]" type="hidden" value="'+row_array[0]+'">'+
				'<div class="operation-section-process-title">'+
				row_array[1]+	
				' <span class="label label-warning">Not Started Yet</span>'+
				'</div>'+
				'<div>'+
				'<div style="padding: 3px; margin-top: 2px;">'+
				'<button type="button" class="btn btn-default btn-xs">Daily Feed</button> '+
				'<button type="button" class="btn btn-default btn-xs">Transfer Stock</button> '+
				'<button type="button" class="btn btn-default btn-xs">View History</button> '+
				'<div>'+
				'<div style="border: 1px solid #ddd; padding: 3px; margin-top: 2px;">'+

				'<div style="text-align: center;"><strong>Stock</strong></div>'+
				'<div class="row">'+
				'<div class="col-md-3"><strong>Received : 100</strong></div>'+
				'<div class="col-md-3"><strong>Processing : 30</strong></div>'+
				'<div class="col-md-3"><strong>Completed : 70</strong></div>'+
				'<div class="col-md-3"><strong>Wastage : 70</strong></div>'+
				'</div>'+
				...
				*/
				...
				// '<div><a href="javascript:;" onclick="addProcessStocks('+row_array[0]+')">Add Item</a> </div>'+
				// '</div>'+
				// '<div class="col-md-3">'+
				// '</div>'+
				'</div>'+
...
$('#addStock').click(function() {
	...
	// var used_qty = $('#used_stock_qty_'+stock_grn_detail_id).val();
	...
	if(exist_qty_exists){
		...
		'<input type="hidden" class="section-'+main_process+'-stock-'+stock_grn_detail_id+'-qty" name="section_stock_qtys['+main_process+'][]" value="'+qty+'" id="used_stock_qty_'+stock_grn_detail_id+'">'+
